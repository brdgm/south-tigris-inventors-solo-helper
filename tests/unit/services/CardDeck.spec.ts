...
describe('services/CardDeck', () => {
  it('new-round-1-level-1', () => {
    ...
    expect(deck.reserve.length, 'reserve').to.eq(5)
  ...
  it('new-round-4-level-3', () => {
    ...
    expect(deck.discard.length, 'discard').to.eq(0)
    expect(deck.reserve.length, 'reserve').to.eq(2)
  })

  it('draw', () => {
    const deck = CardDeck.fromPersistence({ pile: [3,10,2], discard: [4], reserve: [1,6] })
